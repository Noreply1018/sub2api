export default {
  ...
  auth: {
    ...
    loginKeyLabel: '登录密钥',
    loginKeyPlaceholder: '请输入登录密钥',
    loginKeyRequired: '请输入登录密钥',
  ...
  admin: {
    ...
    users: {
      ...
      loginKeySet: '已设置',
      loginKeyNotSet: '未设置',
      loginKeyUnavailable: '未保存明文',
      setLoginKey: '设置登录密钥',
      resetLoginKey: '重置登录密钥',
      clearLoginKey: '清除登录密钥',
      loginKeySetSuccess: '登录密钥已设置',
      loginKeyResetSuccess: '登录密钥已重置，请立即保存新密钥。',
      loginKeyClearSuccess: '登录密钥已清除',
      loginKeyActionFailed: '登录密钥操作失败',
      loginKeyDialogTitle: '设置登录密钥',
      loginKeyDialogHint: '输入后会直接替换该用户首页登录密钥，管理员列表可查看和复制。',
      loginKeyInputPlaceholder: '输入新的登录密钥',
      loginKeyRequired: '请输入登录密钥',
      copyLoginKey: '复制登录密钥',
      loginKeyOneTimeTitle: '新的登录密钥',
      loginKeyOneTimeHint: '该密钥只显示一次，请立即保存并分发给对应用户。',
    ...
    accounts: {
      ...
      groupPriority: {
        ...
        title: '调整分组内账号优先级',
        groupFallback: '当前分组',
        currentPageHint: '当前页可调整 {count} 个账号；数值越小越优先。仅影响当前分组，不影响全局优先级。',
        priority: '分组优先级',
        normalize: '按当前顺序重排',
        toTop: '置顶',
        moveUp: '上移',
      ...
      priority: '全局优先级',
      priorityHint: '默认/兜底优先级；数值越小越优先。具体分组内的调度优先使用分组优先级。',
  ...
  onboarding: {
    ...
    admin: {
      welcome: {
        ...
        personalDescription:
          '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">Sub2API 是一个强大的 AI 服务中转平台，让您轻松管理和分发 AI 服务。</p><p style="margin-bottom: 12px;"><b>🎯 核心功能：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>🔗 <b>账号池</b> - 连接多个上游 AI 服务商账号</li><li>🔑 <b>密钥分发</b> - 为用户生成独立的 API Key</li><li>📊 <b>用量统计</b> - 保留请求和 Token 统计</li></ul><p style="color: #10b981; font-weight: 600;">接下来，我们将用 3 分钟带您完成首次配置 →</p></div>',
        nextBtn: '开始配置 🚀',
        prevBtn: '跳过'
      },
      groupManage: {
