...
- 范围：用户表新增网页登录密钥哈希；后端新增密钥登录接口和管理员密钥生成/清除能力；前端首页登录页改为单密钥输入；“记住我”对密钥登录继续生效。
- 不做：不复用网关 API Key；不删除底层账号密码登录接口、注册、OAuth 绑定和密码相关能力；不改 8080 正式版。
- 验收：8081 上管理员可为用户生成/重置登录密钥；首页输入正确密钥可登录对应用户，错误密钥被拒绝；管理员密钥进入后仍有管理权限；勾选“记住我”后 refresh token 使用长会话 TTL。
- 验证：2026-06-15 在 8081 开发环境完成；`/health` 返回 `{"status":"ok"}`；迁移已创建 `users.login_key_hash` 与 `users_login_key_hash_active_idx`；管理员重置登录密钥后，`/api/v1/auth/key-login` 可登录为 admin；错误密钥返回 `401 INVALID_CREDENTIALS`；勾选 `remember_me=true` 的密钥登录 refresh token TTL 为 `7775963` 秒（约 90 天）；运行包包含“登录密钥”“请输入登录密钥”“重置登录密钥”文案。测试生成的管理员登录密钥已清除。
