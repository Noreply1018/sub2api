...
- 状态：done
- 记录：2026-06-15
- 目标：把首页账号密码登录改为个人密钥登录；管理员和普通用户输入各自密钥后进入对应身份。
- 范围：用户表新增网页登录密钥哈希和加密展示副本；后端新增密钥登录接口和管理员密钥生成/手动设置/清除能力；前端首页登录页改为单密钥输入，管理端可查看、复制、手动设置用户登录密钥；“记住我”对密钥登录继续生效。
- 不做：不复用网关 API Key；不删除底层账号密码登录接口、注册、OAuth 绑定和密码相关能力；不改 8080 正式版。
- 验收：8081 上管理员可为用户生成/手动设置/清除登录密钥；管理员列表可查看之后设置或生成的密钥明文；首页输入正确密钥可登录对应用户，错误密钥被拒绝；管理员密钥进入后仍有管理权限；勾选“记住我”后 refresh token 使用长会话 TTL。
- 验证：2026-06-15 在 8081 开发环境完成；`/health` 返回 `{"status":"ok"}`；迁移已创建 `users.login_key_hash`、`users.login_key_encrypted` 与 `users_login_key_hash_active_idx`；管理员手动设置登录密钥后，管理员用户接口和列表接口返回可展示明文；`/api/v1/auth/key-login` 可登录为 admin，返回的 refresh token 可通过 `/api/v1/auth/refresh` 轮转；错误密钥返回 `401 INVALID_CREDENTIALS`；勾选 `remember_me=true` 的密钥登录走 `GenerateTokenPairWithRememberMe` 长会话路径；运行包包含“登录密钥”“请输入登录密钥”“设置登录密钥”“未保存明文”“输入新的登录密钥”文案。当前 8081 管理员测试登录密钥为 `admin-custom-key-8081-1781493989`。
